...
const openSlide = (slide: SlideData) => {
  // 普通模式
  const url = new URL(`test/${slide.url}/index.html`, window.location.href)
  // IPFS 模式
  // const url = new URL(`/test/${slide.url}`, window.location.href)
  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-100 via-white to-gray-200 py-16 px-4">
    <div class="max-w-5xl mx-auto">
      ...
      <div class="space-y-4 mb-8">
        <div class="relative">
          <Input placeholder="Search by title, description, or author..." v-model="searchTerm" />
        </div>
      </div>
